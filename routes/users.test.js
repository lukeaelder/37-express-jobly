...
describe("POST /users/:username/jobs/:id", () => {
  test("works for admins", async () => {
    const response = await request(app).post(`/users/u1/jobs/${testJobId[0]}`)
        .set("authorization", `Bearer ${adminToken}`);
    expect(response.body).toEqual({applied:testJobId[0]});
  });
  ...
  test("fails if user is not found", async () => {
    const response = await request(app).post(`/users/u0/jobs/${testJobId[0]}`)
        .set("authorization", `Bearer ${adminToken}`);
    expect(response.statusCode).toEqual(404);
  });
  test("fails if job is not found", async () => {
    const response = await request(app).post(`/users/u1/jobs/0`)
        .set("authorization", `Bearer ${adminToken}`);
    expect(response.statusCode).toEqual(404);
  });
});